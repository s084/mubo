...
# MXFP4/Blackwell support - use llama-server instead of Ollama
USE_MXFP4 = os.environ.get("MUBO_MXFP4", "false").lower() == "true"
LLAMA_SERVER_PORT = os.environ.get("MUBO_LLAMA_PORT", "8080")
LLAMA_BASE = f"http://localhost:{LLAMA_SERVER_PORT}"
...
def _llm_generate_simple(prompt: str, options: dict = None) -> str:
    """Generate text using either Ollama or llama-server based on MXFP4 mode."""
    if USE_MXFP4:
...
Output format: one query per line, no numbering, no explanation."""
        ...
        text = _llm_generate_simple(prompt, {"num_predict": 250})
        if text:
            if "</think>" in text:
