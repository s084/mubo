...
") 2>/dev/null || {
    ...
    MUBO_MODEL="${DERIVED_MODEL}" MUBO_PORT="${port}" MUBO_MXFP4="${IS_BLACKWELL}" MUBO_LLAMA_PORT="${LLAMA_SERVER_PORT}" \
...
detect_blackwell() {
    ...
    if [[ "$compute_cap" == "12.0" || "$compute_cap" == "12.1" || "$compute_cap" == "12.2" ]]; then
        IS_BLACKWELL=true
        info "Blackwell GPU detected (compute capability ${compute_cap})"
        info "MXFP4 quantization will be used for 2.5x faster inference"
    fi
}

setup_llama_cpp_mxfp4() {
    if [[ "$IS_BLACKWELL" != true ]]; then
        return
    fi

    step "Setting up llama.cpp with MXFP4 for Blackwell GPU"

    local llama_dir="$HOME/llama-mxfp4"

    # Check if already built
    if [[ -f "${llama_dir}/build/bin/llama-server" ]]; then
        ok "llama.cpp already built for Blackwell"
    else
        # Install CUDA Toolkit 12.9 if needed
        local nvcc_version
        nvcc_version=$(nvcc --version 2>/dev/null | grep "release" | sed "s/.*release \([0-9]*\.[0-9]*\).*/\1/")
        if [[ -z "$nvcc_version" ]] || (( $(echo "$nvcc_version < 12.4" | bc -l) )); then
            info "Installing CUDA Toolkit 12.9 for Blackwell support..."
        ...
        info "Building llama.cpp with Blackwell support..."
        rm -rf "$llama_dir"
        ...
        cd - > /dev/null
        ok "llama.cpp built for Blackwell"
    fi

    # Download MXFP4 model
    local model_file="${llama_dir}/SmallThinker-4B-MXFP4.gguf"
    if [[ -f "$model_file" ]]; then
        ok "MXFP4 model already downloaded"
    else
        info "Downloading SmallThinker-4B MXFP4 model (2.6GB)..."
        curl -L -o "$model_file" \
            "https://huggingface.co/noctrex/SmallThinker-4B-A0.6B-Instruct-MXFP4_MOE-GGUF/resolve/main/SmallThinker-4B-A0.6B-Instruct-MXFP4_MOE.gguf?download=true"
        ok "MXFP4 model downloaded"
    fi

    MXFP4_MODEL_PATH="$model_file"

    # Start llama-server
    info "Starting llama-server with MXFP4 model..."
    export PATH=/usr/local/cuda-12.9/bin:$PATH
    ...
    local sw=0
    while ! curl -s -o /dev/null http://localhost:${LLAMA_SERVER_PORT}/health 2>/dev/null; do
        sleep 1
        sw=$((sw + 1))
        if [[ $sw -ge 30 ]]; then
            warn "llama-server startup timed out"
            return
        fi
    done
